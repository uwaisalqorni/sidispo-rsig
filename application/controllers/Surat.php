...
class Surat extends MY_Controller {
    ...
    public function create()
    {
        ...
        // 1. Tentukan nomor_agenda
        $nomor_agenda_input = $this->input->post('nomor_agenda');
        $has_nomor_agenda   = $this->input->post('has_nomor_agenda');

        $nomor_agenda = '__AUTO__';
        if ($has_nomor_agenda === '0' || $has_nomor_agenda === 'false' || $has_nomor_agenda === false) {
            $nomor_agenda = '__NONE__';
        } elseif (!empty($nomor_agenda_input) && trim($nomor_agenda_input) !== '' && $nomor_agenda_input !== '__AUTO__') {
    ...
    public function update($id = null)
    {
        ...
        if ($content_type && strpos($content_type, 'multipart/form-data') !== false) {
            ...
            $nomor_agenda_input = $input['nomor_agenda'] ?? $this->input->post('nomor_agenda');
            $has_nomor_agenda   = $input['has_nomor_agenda'] ?? $this->input->post('has_nomor_agenda');
        ...
        $update_data = [];
        if ($has_nomor_agenda === '0' || $has_nomor_agenda === 'false' || $has_nomor_agenda === false) {
            $update_data['nomor_agenda'] = null;
        } elseif ($nomor_agenda_input !== null && trim($nomor_agenda_input) !== '') {
            $update_data['nomor_agenda'] = trim($nomor_agenda_input);
        }
