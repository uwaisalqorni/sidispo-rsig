...
class Notifikasi_model extends CI_Model {
     ...
     */
    public function insert($data, $send_email = true)
    {
        $this->db->insert('notifikasi', $data);
        $insert_id = $this->db->insert_id();

        if ($send_email && !empty($data['user_id'])) {
     ...
     */
    public function insert_batch($data, $send_email = true)
    {
        $res = $this->db->insert_batch('notifikasi', $data);
        
        if ($send_email) {
            $this->send_batch_emails($data);
        }
    ...
    /**
     * Kirim email notifikasi secara batch/efisien untuk banyak penerima
     */
    public function send_batch_emails($notif_batch)
    {
        ...
        try {
            // 1. Ambil setting SMTP sekali saja
            ...
            $config = [
                ...
                'smtp_timeout' => 5, // Timeout 5 detik agar tidak menggantung
                'mailtype'     => 'html',
                'charset'      => 'utf-8',
                'newline'      => "\r\n",
                'crlf'         => "\r\n"
            ...
            // 2. Ambil data semua user terkait dalam 1 query
            $user_ids = array_unique(array_filter(array_column($notif_batch, 'user_id')));
            if (empty($user_ids)) {
                return;
            }

            $this->db->select('id, email, nama_lengkap');
            ...
            // 3. Cache detail disposisi
            $disposisi_cache = [];

            foreach ($notif_batch as $item) {
                $uid = $item['user_id'] ?? null;
                if (!$uid || empty($users_map[$uid]['email'])) {
                    continue;
                }

                $user = $users_map[$uid];
                $judul = $item['judul'] ?? 'Notifikasi Disposisi';
                $pesan = $item['pesan'] ?? '';
                $disp_id = $item['disposisi_id'] ?? null;

                $disposisi = null;
                if ($disp_id) {
                    if (!isset($disposisi_cache[$disp_id])) {
                        $this->db->select('
                        ...
                        $this->db->where('d.id', $disp_id);
                        $disposisi_cache[$disp_id] = $this->db->get()->row_array();
                    }
                    $disposisi = $disposisi_cache[$disp_id];
                }
                ...
                $body = $this->build_email_html($user, $judul, $pesan, $disposisi);
                $this->email->message($body);
                @$this->email->send();
            }
        } catch (Exception $e) {
            log_message('error', 'Gagal kirim batch email notifikasi: ' . $e->getMessage());
        }
    }

    /**
     * Kirim email notifikasi jika setting notif_email aktif (single)
     */
    ...
    /**
     * Helper membuat HTML body email
     */
    private function build_email_html($user, $judul, $pesan, $disposisi = null)
    {
        // Styling badges
